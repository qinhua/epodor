...
const Detail: React.FC = () => {
    ...
    return (
      <div className="flex items-center justify-center h-[calc(100vh-80px)]">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-white mb-2">组件未找到</h2>
          <button
  ...
  const [activeTab, setActiveTab] = useState<"visual" | "principle" | "test">(
    "visual"
  );
  ...
  return (
    <div className="h-[calc(100vh-80px)] flex flex-col lg:flex-row gap-6">
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-2 pb-10">
        <button
          onClick={() => navigate("/library")}
          className="flex items-center text-muted hover:text-white transition-colors w-fit group"
        >
          <ArrowLeft
            size={18}
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                  {component.specs.map((spec, idx) => (
                    <div
                      key={idx}
                      className="bg-white/5 p-3 rounded-lg border border-white/5 hover:bg-white/10 transition-colors"
                    >
            ...
            {activeTab === "principle" && (
              ...
              >
                ...
                <h4 className="text-lg font-bold mt-8 mb-4 text-white">
                  常见应用场景
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {component.applications.map((app, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 bg-slate-800/50 p-3 rounded-lg border border-slate-700/50 hover:border-primary/30 transition-colors"
                    >
          ...
          {chatHistory.length === 0 && (
            <div className="text-center text-muted text-sm py-10 px-6">
              ...
              <div className="mt-4 space-y-2">
                <p
                  className="bg-slate-800/50 p-2 rounded cursor-pointer hover:bg-slate-800 transition-colors border border-white/5"
                  onClick={() =>
                    setChatQuery(`如何判断 ${component.name} 是否损坏？`)
                  }
                >
                  "如何判断它是否损坏？"
                </p>
                <p
                  className="bg-slate-800/50 p-2 rounded cursor-pointer hover:bg-slate-800 transition-colors border border-white/5"
                  onClick={() =>
                    setChatQuery(`${component.name} 在电路中起什么作用？`)
                  }
                >
                  "它在电路中起什么作用？"
                </p>
        ...
        <div className="p-4 border-t border-border bg-slate-900">
          <div className="flex gap-2">
            ...
            <button
              ...
              className="bg-primary text-white px-3 rounded-lg hover:bg-cyan-400 disabled:opacity-50 transition-colors shadow-lg shadow-primary/20"
            >
              <Send size={18} />
            </button>
          </div>
